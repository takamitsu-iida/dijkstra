...
#
# A*アルゴリズムは、ダイクストラ法にヒューリスティック関数を追加したものです。
# ダイクストラ法では自分から到達可能な全てのノードへの最短経路を求めますが、
# A*アルゴリズムでは、ゴールとなるノードに向かう最短経路を求めるのに使われます。
# 迷路を解いたり、ゲームのキャラクタ操作を自動化するのに使われることが多いアルゴリズムです。
#
# ヒューリスティック関数は、そのノードからゴールまでの最短距離の見積もりを返す関数です。
# この関数をh(n)とすると、A*アルゴリズムでは、
# f(n) = g(n) + h(n)
# という評価関数を用いて、最小のf(n)を持つノードを選択します。
# ここで、g(n)は始点からノードnまでの最短距離です。
# g(n)はダイクストラ法と同じなので、h(n)が常に0を返す関数なら、A*アルゴリズムはダイクストラ法と同じになります。
#

# (.venv) iida@FCCLS0073460:~/git/dijkstra$ bin/a_star.py
# --- fig-3-6.json ---
# [['s', 'B', 'F', 't']]
#
# --- fig-3-7.json ---
